    ...
    this.material = new RawShaderMaterial({
      ...
      fragmentShader: `
            ...
            void main() {
              ...
              #ifdef UINT_TARGET
              vec4 x0 = texture2D(previousIteration, vUV - vec2(texelSize.x, 0)) * 2.0 - 1.0;
              vec4 x1 = texture2D(previousIteration, vUV + vec2(texelSize.x, 0)) * 2.0 - 1.0;
              vec4 y0 = texture2D(previousIteration, vUV - vec2(0, texelSize.y)) * 2.0 - 1.0;
              vec4 y1 = texture2D(previousIteration, vUV + vec2(0, texelSize.y)) * 2.0 - 1.0;
              vec4 d = texture2D(divergence, vUV) * 2.0 - 1.0;

              gl_FragColor = (x0 + x1 + y0 + y1 + alpha * d) * beta * 0.5 + 0.5;
              #else
